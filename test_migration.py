...
v12_dict = open('v12_salida.txt', 'r')
v13_dict = open('v13_salida.txt', 'r')

differences_file = open('migration_differences.txt', 'w')



def set_data():
    for linea in v12_dict:
        res = json.loads(linea)
        data_object.data_12 = eval(res)

    for linea in v13_dict:
        res = json.loads(linea)
        data_object.data_13 = eval(res)

def compare_index(table_names):
    
    tablas_identicas = 0
    tablas_inexistentes = 0
    tablas_diferentes  = 0

    output_text = ''
    for table_name in table_names:
        try:
            difference = set(data_object.data_13[table_name]).symmetric_difference(set(data_object.data_12[table_name]))
            output_text += '\n %s \n' % table_name
            if len(difference) > 0:
                output_text += str(difference) + '\n'
                tablas_diferentes += 1
            else:
                output_text += 'tablas identicas' + '\n'
                tablas_identicas += 1

        except:
            output_text += '\n %s \n' % table_name
            output_text += 'la tabla no existe en uno de los dos esquemas de BD \n'
            tablas_inexistentes += 1
    
    output_text += '\n RESUMEN \n'
    output_text += 'TABLAS IGUALES %s \n TABLAS DIFERENTES %s \n TABLAS INEXISTENTES %s \n' % (tablas_identicas, tablas_diferentes, tablas_inexistentes)

    differences_file.writelines(output_text)
...
'account_incoterms',
'account_invoice',
'account_invoice_account_invoice_send_rel',
...
'sms_send_sms',
'snailmail_letter',
'snailmail_letter_account_invoice_send_rel',
'stock_picking',
